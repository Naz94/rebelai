...
import { generateCopy, extractTopic,
         adaptForInstagram }                          from "../../lib/generate.js";
import { generateVisual }                            from "../../lib/visual.js";
import { ROTATIONS, pickAngle, pickHookStyle }       from "../../lib/rotations.js";
...
export default async function handler(req, res) {
  ...
  if (action === "regenerate") {
    ...
    // Pick fresh angle + hook for this regeneration (hook weights bias selection)
    ...
    // Instagram copy adapted from it (same message, compressed, 25 hashtags).
    ...
    // Generate a fresh image based on the new copy.
    // Uses draft.runId so the Blob filename stays traceable to the original run.
    const [topic, visual] = await Promise.all([
      extractTopic(facebookCopy),
      generateVisual(rotation, facebookCopy, draft.runId),
    ]);
    ...
    const newDraft = await saveDraft({
      ...
      visual: {
        ...
        // buffer intentionally omitted — Blob URL is the source of truth
      },
      status:    "pending",
