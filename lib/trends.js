...
//
// Builds a daily "what to write about now" brief and stores it in Redis.
// Injected into every generation call to keep messaging current.
//
// Signal sources (zero API keys required):
//   1. Hacker News Algolia API  — top stories from the last 24h
//   2. Dev.to API               — top articles today
//   3. Brand resource snapshot  — own blogs, tools, projects
//
// Output structure:
//   MOMENTUM: one sentence
//   ANGLE 1 (Contrarian): actionable post angle
//   ANGLE 2 (Breakdown): different category
//   ANGLE 3 (Hidden Cost): founder/operator angle
//   HOOK A: direct opening line, no questions
//   HOOK B: second opening line, different structure
// ============================================================
...
// Pulls top stories from HN and Dev.to — no API keys needed.
// Filters to dev/tech relevant titles only.
// Returns array of { title, source, url } objects.
// ─────────────────────────────────────────────────────────────
...
async function fetchLiveSignals() {
  ...
  return signals.slice(0, 20); // cap at 20 for prompt size
}
...
async function fetchHackerNews() {
  try {
    ...
    console.warn("[trends] HN fetch failed — skipping");
    return [];
  }
}
...
async function fetchDevTo() {
  try {
    ...
    console.warn("[trends] Dev.to fetch failed — skipping");
    return [];
  }
}

// Filters to developer/infrastructure/AI relevant content only.
// Excludes politics, sports, general news.
const DEV_KEYWORDS = [
...
// ─────────────────────────────────────────────────────────────
// AI BRIEF GENERATION
// Synthesises live signals + brand assets into a structured brief.
// Angle types enforced — prevents three same-feeling angles.
// ─────────────────────────────────────────────────────────────
