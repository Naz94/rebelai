...
async function fetchMetaInsights(postId, platform) {
  try {
    if (platform === "instagram") {
      ...
      // Facebook photo posts: likes + comments edges only — reach not available on photo objects
      ...
      console.log(`[performance] FB photo insights ${postId}: likes=${likes} comments=${comments} shares=${shares}`);
      return { likes, comments, saves: 0, reach: 0, impressions: 0, shares };
    }
  } catch (err) {
