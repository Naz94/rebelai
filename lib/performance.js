...
// Must match the rotation IDs in lib/rotations.js exactly.
// Higher = selected more often before performance data exists.
// These are overridden by live engagement data over time.
// ─────────────────────────────────────────────────────────────
...
const BASE_ROTATION_WEIGHTS = {
  ...
  ai_whatsapp:      65,   // WhatsApp bot sell — highest priority
  ai_social:        60,   // social media agent sell
  trust_process:    50,   // demystify the process
  trust_signals:    50,   // ownership / no lock-in
  sa_business:      45,   // South African context
  client_reality:   50,   // empathy / make them feel seen
  trust_builder:    30,   // founder voice — lower frequency
...
// Merges live Redis weights on top of base weights.
// Only keys present in BASE_ROTATION_WEIGHTS are used —
// stale keys from old rotation IDs are silently ignored.
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// RE-WEIGHT rotations based on average engagement score
// Only updates IDs that exist in BASE_ROTATION_WEIGHTS.
// ─────────────────────────────────────────────────────────────
...
async function reweightRotations() {
  ...
  for (const [, data] of Object.entries(allScores)) {
    ...
    // Only track IDs that are in the current rotation library
    if (!r || !(r in BASE_ROTATION_WEIGHTS)) continue;
  ...
  console.log("[performance] Rotation weights updated:", newWeights);
}
...
async function reweightHooks() {
  ...
  console.log("[performance] Hook weights updated:", newWeights);
}
