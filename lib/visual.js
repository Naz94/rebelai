...
// image URL expires in ~1 hour and cannot be used at publish time.
// ============================================================
...
Do NOT attempt to render full sentences or long phrases — the image model will distort them into gibberish.
...
async function buildSubjectDescription(postContent, imageMode) {
  const modeHints = {
    ...
    minimal_graphic: "Extract the single sharpest 2–4 word phrase from this post. Something that works as large bold text on a dark background. Must be 4 words or fewer — the image model cannot render longer text without errors.",
...
// permanent public URL. Called immediately after image
...
export async function generateVisual(rotation, postContent = "") {
  ...
  const imageUrl_raw = await generateImage({ ...rotation, imagePrompt: fullPrompt });
  const buffer       = await downloadImage(imageUrl_raw);

  // Upload to Blob immediately — OpenAI image URLs expire in ~1 hour
  // The Blob URL is permanent and publicly accessible by Instagram
  let imageUrl = imageUrl_raw; // fallback if Blob upload fails
  try {
    imageUrl = await uploadToBlob(buffer, "image/png");
  } catch (err) {
    console.warn("[visual] Blob upload failed, falling back to OpenAI URL (may expire):", err.message);
  }
  ...
  return {
    ...
    imageUrl,  // permanent Blob URL (or OpenAI URL if Blob failed)
  };
}
