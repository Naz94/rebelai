...
export async function saveDraft(data) {
  // Always generate a fresh ID — never inherit one from a spread
...
export async function getAllDrafts() {
  // Get up to 200 IDs from the queue list
  const ids = await redis.lrange(QUEUE_KEY, 0, 199);
  if (!ids || ids.length === 0) return [];

  // Fetch each draft individually to avoid hmget field-count mismatches
  const results = await Promise.all(
    ids.map(id => getDraft(id))
  );
