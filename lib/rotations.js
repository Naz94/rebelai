...
Tools available (both live on the homepage at rebeldesigns.co.za):
- Password Roast: rates password strength with brutal, witty commentary. Engineering angle: real-time entropy calculation, UX that makes security feedback non-boring, decision to use client-side only so no passwords are ever transmitted.
- Rebel T-T-T: a precision-designed tic-tac-toe game. Engineering angle: minimax algorithm implementation, UI interaction design at a high level, what a "simple" game reveals about component architecture when built properly.
...
- Include ONE link: rebeldesigns.co.za (this is where both tools live — do not invent a subdirectory)
