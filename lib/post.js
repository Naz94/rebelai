...
// If imageUrl is expired or missing, upload the buffer to Vercel
// Blob to get a fresh permanent public URL.

async function resolveInstagramUrl(visual, imageBuffer) {
  // Already have a URL — verify it's accessible
  if (visual?.imageUrl) {
    try {
      const check = await fetch(visual.imageUrl, { method: "HEAD" });
      if (check.ok) return visual.imageUrl;
      console.warn("[post] imageUrl returned", check.status, "— re-uploading to Blob");
    } catch (err) {
      console.warn("[post] imageUrl HEAD check failed:", err.message, "— re-uploading to Blob");
    }
  }

  // URL missing or expired — upload buffer to Blob
  if (imageBuffer) {
    try {
      console.log("[post] Uploading buffer to Blob for Instagram");
      const mimeType = visual?.mimeType ?? "image/png";
      const ext      = mimeType === "image/jpeg" ? "jpg" : "png";
      const filename = `rebelai/visuals/ig-upload-${Date.now()}.${ext}`;
