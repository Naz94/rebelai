...
export async function generateCopy(rotation, platform, resourceSnapshot, postHistory = [], intelligenceBrief = null, overrides = {}) {
  ...
  do {
    ...
    const tokenCap = platform === "facebook" ? 500 : 250;

    const response = await client.chat.completions.create({
      model:       "gpt-4o",
      max_tokens:  tokenCap,    // facebook: ~380 words headroom · instagram: ~190 words headroom
      temperature: 0.85,        // enough variation for freshness, tight enough to hold brand voice
      messages: [
...
const BANNED_PHRASES = [
  ...
  "game-changer",
  "—",              // em-dash — brand voice rule: use short sentences instead
  "the saveable insight:",   // model labelling its own insight — just make the point
  "saveable insight:",
  "echoes through",          // filler/cringe openers
...
function passesQualityGate(copy, platform = "facebook") {
  ...
  // ── Word count ceiling ───────────────────────────────────
  // Count words in the caption body only (before the hashtag block).
  // Hashtags are on their own line block — split them out first.
  const bodyOnly   = copy.split(/\n(?=#)/)[0].trim();  // everything before first hashtag line
  ...
  // Instagram prompt asks for exactly 25. Allow 23–26 as tolerance.
