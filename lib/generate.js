...
export async function generateCopy(rotation, platform, resourceSnapshot, postHistory = [], intelligenceBrief = null, overrides = {}) {
  ...
  do {
    ...
    if (passesQualityGate(copy) && passesHallucinationCheck(copy)) break;
...
function passesQualityGate(copy) {
  if (!copy || copy.length < 80) return false;  // Instagram posts can be short — 80 chars is the real floor
