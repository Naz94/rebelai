...
function passesQualityGate(copy, platform = "facebook", rotationType = "value") {
  ...
  // ── Repetition check ─────────────────────────────────────
  // Rejects posts where the same core claim appears more than twice.
  // Checks for sentences sharing 5+ meaningful words — catches
  // "catches tampered PDFs" / "stops fraud manual inspection misses" / "tool stops fraud" patterns.
  ...
  for (let i = 0; i < sentences.length; i++) {
    for (let j = i + 1; j < sentences.length; j++) {
      ...
      if (shared.length >= 5) {
        console.warn("[generate] Repetition detected — retrying");
        return false;
      }
    }
  }
