...
function passesQualityGate(copy, platform = "facebook", rotationType = "value") {
  if (!copy || copy.length < 80) {
    console.warn("[gate] FAIL: copy too short");
    return false;
  }
  ...
  for (const pattern of BANNED_OPENERS) {
    if (pattern.test(firstLine)) {
      console.warn(`[gate] FAIL banned opener: ${pattern} matched "${firstLine.slice(0, 60)}"`);
      return false;
    }
  }
  ...
  for (const phrase of BANNED_PHRASES) {
    if (copy.toLowerCase().includes(phrase.toLowerCase())) {
      console.warn(`[gate] FAIL banned phrase: "${phrase}"`);
      return false;
    }
  }

  if (rotationType === "lab" && endsWithQuestion(copy)) {
    console.warn("[gate] FAIL: lab post ends with question");
    return false;
  }
  ...
  if (wordCount > wordLimit) {
    console.warn(`[gate] FAIL: word count ${wordCount} > ${wordLimit}`);
    return false;
  }
  ...
  if (platform === "instagram") {
    ...
    if (hashtagMatches.length < 23 || hashtagMatches.length > 26) {
      console.warn(`[gate] FAIL: hashtag count ${hashtagMatches.length} not in range 23-26`);
      return false;
    }
  }
  ...
  for (let i = 0; i < sentences.length; i++) {
    for (let j = i + 1; j < sentences.length; j++) {
      ...
      if (shared.length >= 5) {
        console.warn(`[gate] FAIL repetition: "${sentences[i].slice(0, 50)}" / "${sentences[j].slice(0, 50)}"`);
        return false;
      }
    }
  }
