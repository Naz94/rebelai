...
export async function generateCopy(rotation, platform, resourceSnapshot, postHistory = [], intelligenceBrief = null, overrides = {}) {
  ...
  // rotation.type is passed so the closing-question rule only fires on lab posts.
  // Value posts (value_react, value_perf, etc.) are REQUIRED to end with a
  // question by their rotation brief — banning it on all types was causing
  // the retry loop to exhaust all attempts on every value post.
    ...
    try {
      const response = await withOpenAIRetry(() =>
        client.chat.completions.create({
          model:      "gpt-4o",
          max_tokens: 350,           // Facebook <=220w, Instagram <=110w — 350 tokens is sufficient
          messages: [
      ...
      console.error(`[generate] OpenAI call failed on attempt ${attempts}: ${err.message}`);
      if (attempts >= 3) throw err;
      await sleep(RETRY_DELAY_MS * attempts);
    }
  } while (attempts < 3);

  if (attempts === 3 && copy.length < 80) {
    throw new Error(`[generate] Failed to produce valid copy after ${attempts} attempts`);
  }
...
async function withOpenAIRetry(fn, maxRetries = 2) {
  let lastErr;
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    try {
      ...
      console.warn(`[generate] OpenAI retry ${attempt}/${maxRetries + 1}: ${err.message}`);
...
The opening line must embody this hook style — do not save it for later in the post.`;
  ...
  // Value posts MUST end with a question (community engagement).
  // Lab posts MUST end with a CTA (drive traffic to the tool/resource).
...
//   - "value" posts MUST end with a question (rotation brief requires it)
...
const BANNED_OPENERS = [
  ...
  /^here is your post/i,
  /^here['']s your post/i,
  /^here['']s why/i,                  // "Here's why:" filler opener
  /^the (rise|power|key|secret|truth|reality|problem|answer) of/i,
  /^complexity (does not|doesn['']t)/i,
  /^manual processes are/i,
];
...
const BANNED_PHRASES = [
  ...
  // Structure-narrating phrases (model explaining its own format)
  ...
  // Padding phrases caught in live output
  ...
  // Hedging and vague transformation language
  "sounds effective",
  "seems efficient",
  "appears efficient",
  "effortlessly",
  ...
  "syntactical sugar",
  "utility fetishism",
  "design with observation",
  "leave no corner unobserved",
  "is not paranoia",
  "it is pragmatism",

  // Corporate safe talk caught in live output
  "enabling systems to",
  "allows developers to",
  "ensures smooth operations",
  "without human error creeping in",
  "transforming them from liabilities into assets",
  "harness tailwind's real value",
  "harness the real value",
  "to harness",
  "prioritize component abstraction",
];
...
function passesQualityGate(copy, platform = "facebook", rotationType = "value") {
  ...
  // Lab posts must end with a CTA, not a question.
  // Value posts must end WITH a question — do NOT apply this check to them.
...
export async function extractTopic(copy) {
  ...
  try {
    ...
    console.warn("[generate] Topic extraction failed:", err.message);
    // Fallback: extract first sentence
    return copy.split(/[.!?]/)[0].trim().slice(0, 80) || "Content generation";
  }
}
